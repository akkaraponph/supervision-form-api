...
const CFQSectionModel = db.CFQSubSection
const ResultCFOEQSubSectionModel = db.ResultCFOEQSection

export const create = async (req: Request, res: Response) => {
    try {
        const payload = await ResultCFOEQSubSectionModel.create(req.body)
...
export const getOne = async (req: Request, res: Response) => {
    try {
        const payload = await ResultCFOEQSubSectionModel.findOne({
...
export const getAll = async (req: Request, res: Response) => {
    try {
        const payload = await ResultCFOEQSubSectionModel.findAll({
            include : [
                {
                    model: CFQSectionModel,
                }
...
export const update = async (req: Request, res: Response) => {
    try {
        const payload = await ResultCFOEQSubSectionModel.update({
            ...req.body,
        },
            {
                where: { id: req.params.id }
...
export const destroy = async (req: Request, res: Response) => {
    try {
        const valid = await ResultCFOEQSubSectionModel.findOne({ where: { id: req.params.id } })
        if (!valid) return res.status(400).json({
            msg: `The data where ${req.params.id} not found`,
            payload: {}
        })
        const payload = await ResultCFOEQSubSectionModel.destroy({
            where: {
                id: req.params.id
            }
        })
