...
import {
  FormType,
  SupervisionFormTypeAttributes,
  supervisionFormTypeEnum,
} from "../supervision-form-type.types";
...
export const getOne = async (req: Request, res: Response) => {
  try {
    ...
    let whereClauseSupervisionFormType: Partial<SupervisionFormTypeAttributes> =
      {}; // Initialize an empty object for the where clause

    if (query.type) {
      whereClauseSupervisionFormType.type =
        query.type as supervisionFormTypeEnum; // Add a condition for the "term" query parameter
    }
    if (query.form_type) {
    ...
    let whereClauseSchoolSupervisionForm: Partial<SchoolSupervisionFormAttributes> =
      {}; // Initialize an empty object for the where clause
    ...
    if (query.supervision_form_id) {
      whereClauseSchoolSupervisionForm.supervisionFormId =
        query.supervision_form_id as string; // Add a condition for the "term" query parameter
    }
...
export const getOneByTermAndYearBySchoolId = async (
  req: Request,
  res: Response
) => {
  try {
...
export const getOneByTermAndYearByPersonnelId = async (
  req: Request,
  res: Response
) => {
  try {
    ...
    if (!user)
      return res.status(400).json({
        msg: `🚀Encoutered an error when retrieved the personnel supervision form by ${req.query.school_id} user was not found`,
        payload: {},
      });

    const personnel = await db.Personnel.findOne({ where: { userId: user }, raw:true});
    if (!personnel)
      return res.status(400).json({
        msg: `🚀Encoutered an error when retrieved the personnel supervision form by ${req.query.school_id} personnel was not found`,
        payload: {},
      });

    const payload = await db.PersonnelSupervisionForm.findOne({
      include: [
        {
    ...
    return res.status(200).json({
      msg: "🚀retrieved the data of personnel supervision form was successfully",
      payload,
    });
  } catch (error) {
	  console.log("-----------------------");
    ...
    return res.status(400).json({
      msg: `🚀Encoutered an error when retrieved the personnel supervision form by ${req.query.school_id}`,
      payload: {},
    });
  }
};
...
export const getAllSchoolReport = async (req: Request, res: Response) => {
  try {
    ...
    const schoolAnswer = await db.SchoolSupervisionForm.findAll({
      ...
      where: {
        [db.Sequelize.Op.and]: [
          ...
          { term },
          {
            "$SupervisionForm.SupervisionFormType.type$": {
    ...
    schoolAnswer.forEach((element: any) => {
      ...
      resultQuestionArray[element["ResultRSFs.RSFQuestion.question"]].push(
        element["ResultRSFs.score"]
      );
...
const getAllSchoolReportByPersonnel = async (req: Request, res: Response) => {
  try {
    ...
    const personnelAnswer = await db.PersonnelSupervisionForm.findAll({
      ...
      where: {
        [db.Sequelize.Op.and]: [
          ...
          // { term },
          {
            "$SchoolSupervisionForm.SupervisionForm.SupervisionFormType.type$":
              { [db.Sequelize.Op.ne]: null },
          },
        ],
        // term, year,
    ...
    personnelAnswer.sort((a: any, b: any) => {
      const priorityA =
        a["PersonnelResultRSFs.RSFQuestion.RSFSection.priority"];
      const priorityB =
        b["PersonnelResultRSFs.RSFQuestion.RSFSection.priority"];
      return priorityA - priorityB;
    });
    ...
    personnelAnswer.forEach((element: any) => {
      if (
        !resultQuestionArray[
          element["PersonnelResultRSFs.RSFQuestion.question"]
        ]
      ) {
        resultQuestionArray[
          element["PersonnelResultRSFs.RSFQuestion.question"]
      ...
      resultQuestionArray[
        element["PersonnelResultRSFs.RSFQuestion.question"]
      ].push(element["PersonnelResultRSFs.score"]);

      const sectionType =
        element["PersonnelResultRSFs.RSFQuestion.RSFSection.type"];
...
export const getAllReportByPersonnel = async (req: Request, res: Response) => {
  try {
    ...
    const allPersonnelSupervisonForm =
      await db.PersonnelSupervisionForm.findAll({
        include: [
          {
            model: db.SchoolSupervisionForm,
            include: [
    ...
    allPersonnelSupervisonForm.map(async (resp: any) => {
      const answer = await db.PersonnelResultRSF.findOne({
        where: { personnelSupervisionFormId: resp.id },
        raw: true,
      });
      if (answer) {
        countScool += 1;
...
export const getAll = async (req: Request, res: Response) => {
  try {
    ...
    const req_endpoint = `${req.protocol}://${req.get("host")}${
      req.originalUrl
    }`;
    let whereClauseSupervisionFormType: Partial<SupervisionFormTypeAttributes> =
      {}; // Initialize an empty object for the where clause
...
export const update = async (req: Request, res: Response) => {
  try {
    ...
    if (!afterUpdateSSF.isSend) {
      const personnelSupervisionForm =
        await db.PersonnelSupervisionForm.findOne({
          where: {
            schoolSupervisionFormId: resp.id,
          },
          raw: true,
...
export const destroy = async (req: Request, res: Response) => {
  try {
    const valid = await SchoolSupervisionForm.findOne({
...
export default {
  ...
  getOneByTermAndYearByPersonnelId,
};
