...
const PersonnelResultRSFModel = db.PersonnelResultRSF
...
export const create = async (req: Request, res: Response) => {
    try {
        ...
        */
        const { RSFQuestionId, personnelSupervisionFormId } = req.body
        const isExist = await PersonnelResultRSFModel.findOne({
            where: {
                RSFQuestionId, personnelSupervisionFormId
            }
        })
        if (isExist) {
            const payload = await PersonnelResultRSFModel.update({
                ...req.body
            }, {
                where: {
                    RSFQuestionId, personnelSupervisionFormId
            ...
            const payload = await PersonnelResultRSFModel.create(req.body)
            return res.json({
...
export const getOneByQuestionId = async (req: Request, res: Response) => {
    try {
        const schoolSupervisionForm = await db.SchoolSupervisionForm.findOne({
            where: { id: req.query.ssid },
            raw: true
        })
        const personnelSupervisionForm = await db.PersonnelSupervisionForm.findOne({
            where: {
                schoolSupervisionFormId: schoolSupervisionForm.id,
            }, raw: true
        })

        const payload = await PersonnelResultRSFModel.findOne({
            where: {
                RSFQuestionId: req.params.id,
                personnelSupervisionFormId: personnelSupervisionForm.id
            },
            include: [
                {
...
export const getAll = async (req: Request, res: Response) => {
    try {
        const payload = await PersonnelResultRSFModel.findAll({
            include: [
                {
                    model: RSFQuestionModel,
                }
...
export const destroy = async (req: Request, res: Response) => {
    try {
        const valid = await PersonnelResultRSFModel.findOne({ where: { RSFQuestionId: req.query.question_id, schoolSupervisionFormId: req.query.ssid } })
        if (!valid) return res.status(204).json({
            msg: `The data where ${req.query.question_id} not found`,
            payload: {}
        })
        const payload = await PersonnelResultRSFModel.destroy({
